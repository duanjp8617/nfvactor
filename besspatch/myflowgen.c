...
struct flowgen_priv {
	...
	double flow_arrival_interval; /*the length of the interval that the flow arrives, in milliseconds*/
	// only care about flow_duration, flow_pps, concurrent_flows and flow_arrival_interval

	uint64_t generated_pkts;
...
const double my_pareto_tail_limit = 0.99;
...
{
	return pow(1.0 / (1.0 - y * my_pareto_tail_limit), inversed_alpha);
}
...
static void populate_initial_flows(struct flowgen_priv *priv)
{
	...
	f = schedule_flow(priv, now_ns+one_sec);
	assert(f);

	uint64_t inter_flow_time = (uint64_t)((priv->flow_arrival_interval*1e6)/priv->concurrent_flows);

	uint64_t total_flows = priv->concurrent_flows;
	uint64_t i = 0;
	for(i=1; i<total_flows; i++){
		now_ns = rdtsc() / tsc_hz * 1e9;
		f = schedule_flow(priv, now_ns+(i*inter_flow_time)+one_sec );
		assert(f);
	}
}
...
process_arguments(struct flowgen_priv *priv, struct snobj *arg)
{
	...
	if ((t = snobj_eval(arg, "flow_arrival_interval")) == NULL)
		return snobj_err(EINVAL, "must specify 'flow_arrival_interval'");
	priv->flow_arrival_interval = snobj_number_get(t);
	if (isnan(priv->flow_arrival_interval) || priv->flow_arrival_interval < 10.0)
		return snobj_err(EINVAL, "invalid 'flow_arrival_interval'");

	if ((t = snobj_eval(arg, "flow_pps")) == NULL)
		return snobj_err(EINVAL, "must specify 'flow_pps'");
	priv->flow_pps = snobj_number_get(t);
	if (isnan(priv->flow_pps) || priv->flow_pps < 1.0)
		return snobj_err(EINVAL, "invalid 'flow_pps'");

	if ((t = snobj_eval(arg, "flow_duration")) == NULL)
		return snobj_err(EINVAL, "must specify 'flow_duration'");
	priv->flow_duration = snobj_number_get(t);
	if (isnan(priv->flow_duration) || priv->flow_duration < 1.0)
		return snobj_err(EINVAL, "invalid 'flow_duration'");

	if ((t = snobj_eval(arg, "concurrent_flows")) == NULL)
		return snobj_err(EINVAL, "must specify 'concurrent_flows'");
	priv->concurrent_flows = snobj_number_get(t);
	if (isnan(priv->concurrent_flows) || priv->concurrent_flows < 1.0)
		return snobj_err(EINVAL, "invalid 'concurrent_flows'");
...
static struct snobj *flowgen_init(struct module *m, struct snobj *arg)
{
	...
	priv->flow_arrival_interval = 1000;
	priv->generated_pkts = 0;
...
static struct snobj *flowgen_get_dump(const struct module *m)
{
...

	{
		...
		snobj_map_set(t, "generated_pkts",
						snobj_int(priv->generated_pkts));
...
static const struct mclass myflowgen = {
	.name 		= "MyFlowGen",
